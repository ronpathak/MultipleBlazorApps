...
namespace MultipleBlazorApps.Client.Helpers
{
    public class HttpServices : IHttpService
    {
        ...
        {
            Console.WriteLine("http service received received request");
    ...
    public async Task<HttpResponseWrapper<object>> Put<T>(string url, T data)
    {
        ...
        public async Task<HttpResponseWrapper<object>> Patch<T>(string url, T data)
        {
            //var dataJson = "[{\"op\":\"replace\",\"path\":\"/ArchiveFlag\",\"value\":\""+data+"\"}]";
            var dataJson = "["+JsonSerializer.Serialize(data)+"]";
            var stringContent = new StringContent(dataJson, Encoding.UTF8, "application/json");
